...
async function sharePrescription(patient: Patient, rx: Prescription) {
  const meds = rx.medications.map(m => `
    <tr>
      <td style="padding:8px 4px;border-bottom:1px solid #eee;font-weight:600">${m.name}</td>
      <td style="padding:8px 4px;border-bottom:1px solid #eee">${m.dosage}</td>
      <td style="padding:8px 4px;border-bottom:1px solid #eee">${m.frequency}</td>
      <td style="padding:8px 4px;border-bottom:1px solid #eee">${m.duration}</td>
    </tr>`).join('');

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
  <style>
    body{font-family:sans-serif;padding:32px;color:#1A2138;max-width:600px;margin:0 auto}
    h1{color:#208AEF;font-size:22px;margin:0 0 4px}
    .subtitle{color:#6B7A99;font-size:13px;margin:0 0 24px}
    .section{margin-bottom:20px}
    .label{font-size:11px;text-transform:uppercase;color:#A0ABBE;letter-spacing:.5px;margin-bottom:4px}
    .value{font-size:14px}
    table{width:100%;border-collapse:collapse;margin-top:12px}
    th{text-align:left;padding:8px 4px;font-size:11px;text-transform:uppercase;color:#A0ABBE;border-bottom:2px solid #E5E9F0}
    .notes{background:#F5F7FA;border-radius:8px;padding:12px;font-size:13px;color:#6B7A99;margin-top:16px}
    .footer{margin-top:32px;font-size:11px;color:#A0ABBE;text-align:center;border-top:1px solid #E5E9F0;padding-top:16px}
  </style></head><body>
  <h1>Prescription</h1>
  <p class="subtitle">Generated by SolvyMed</p>
  <div class="section">
  ...
  <div class="section">
    ...
    <table>
      <thead><tr>
        <th>Name</th><th>Dosage</th><th>Frequency</th><th>Duration</th>
      </tr></thead>
      <tbody>${meds}</tbody>
    </table>
  </div>
  ${rx.notes ? `<div class="notes"><strong>Notes:</strong> ${rx.notes}</div>` : ''}
  <div class="footer">SolvyMed — Clinic Management</div>
  </body></html>`;
                  ...
                  <View key={rx.id} style={styles.rxCard}>
                    <View style={styles.rxHeader}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => sharePrescription(patient, rx)}
                      >
