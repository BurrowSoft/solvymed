...
// medium: where the button sits, recorded as Play install attribution.
export function AppDownloadButtons({ medium = "landing" }: { medium?: string }) {
    ...
    <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
      ...
      {ios.mode === "soon" && (
        <div className="relative w-full sm:w-auto">
          <a
            ...
            aria-label={t("appStoreComingSoon")}
          ...
          >
            ...
            {t("appStore")}
            {/* Coming soon badge */}
            <span aria-hidden="true" className="absolute -top-2.5 -right-2.5 rounded-full bg-amber-400 px-2 py-0.5 text-xs font-bold text-amber-900 shadow-sm whitespace-nowrap">
              {t("soon")}
            </span>
          </a>
        </div>
      ...
      {ios.mode === "beta" && (
        <a
          ...
          className="flex w-full flex-col items-center justify-center rounded-xl bg-teal-500 px-8 py-3 text-white shadow-xl shadow-teal-900/40 transition hover:bg-teal-400 sm:w-auto"
        >
