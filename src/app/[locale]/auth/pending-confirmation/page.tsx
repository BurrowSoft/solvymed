  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: profRowRaw } = await supabase
        .rpc("get_professional_public_info", { p_professional_id: roleRow.invited_by_professional_id })
        .maybeSingle();
    ...
    <AuthPageShell>
      <AuthCard centered>
        ...
        <button
          ...
          disabled={checking}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-teal-600 px-6 py-3.5 text-base font-bold text-white shadow-md shadow-teal-600/20 transition hover:bg-teal-700 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed mb-4"
        >
          {checking ? (
