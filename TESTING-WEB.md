...
| B-7c | Clinical dates near midnight: timezone. **Launch-blocking** (UX, 2026-09-26: the code reviewer's finding) | **When:** run this only **after both fixes are on prod**: web dev's clinicTime PR and mob dev's DB-side date fix. Do one real-wall-clock run at 23:30 BRT (02:30 UTC) and one at 00:30 BRT (03:30 UTC). The bug is already confirmed by code analysis, so it isn't reproduced first. Clock-mocked unit tests in the fix PR cover the same cases, and this run is the final proof. ❌ blocks the launch. With the browser in `America/Sao_Paulo`, create a **record**, a **prescription** and a **correction** at **23:30 Brazil time** (02:30 UTC the next day). Each must show **today's Brazil date**, not tomorrow's, in the chart (the patient's Records and Prescriptions tabs) **and** in any export or PDF of it. Repeat at **00:30 Brazil time** (03:30 UTC, same UTC date): each must show the **new** day. **Extra case:** the browser in `Asia/Bangkok` for a Brazil clinic. The stored and displayed date must be the clinic's Brazil date, not the browser's. **How to run it:** the server and DB clocks can't be faked (`add_record_correction` stamps `current_date` on the DB, in UTC), so run this at the real wall-clock times, 23:30 and 00:30 BRT. Only the browser time can be pinned (Playwright `timezoneId`). Check the stored `date`/`time` columns through REST, as well as what the UI shows. |
